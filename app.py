...
class HullWhite:
    def __init__(self):
        ...
        self.ax_y = Axis(
            label="Rate", scale=self.y_sc, orientation="vertical", grid_lines="dashed"
        )
        ...
        self.line2 = Lines(
            x=[], y=[[]], scales={"x": self.x_sc, "y": self.y_sc}, labels=["MC"]
        )
        self.hist1 = Hist(
            sample=[], scales={"sample": self.x_sc_2, "count": self.y_sc_2}, bins=0
        )
        self.fig1 = Figure(
        ...
        self.fig1.layout.width = chart_width

        self.fig2 = Figure(
            axes=[self.ax_x_2, self.ax_y_2],
            marks=[self.hist1],
            title="Final Distribution of Rates",
        )
        self.fig2.layout.width = chart_width
        ...
        self.fig3.layout.width = chart_width
        ...
        self.fig4.layout.width = "500px"

        # Input widgets
        layout_kwargs = {"width": "140px", "height": "100%", "margin": "5px"}
        self.ZC1W = ipw.FloatText(
            description="1W", value=2.07, layout=ipw.Layout(**layout_kwargs)
        )
        self.ZC3M = ipw.FloatText(
            description="3M", value=2.29, layout=ipw.Layout(**layout_kwargs)
        )
        self.ZC6M = ipw.FloatText(
            description="6M", value=2.45, layout=ipw.Layout(**layout_kwargs)
        )
        self.ZC1Y = ipw.FloatText(
            description="1Y", value=2.7, layout=ipw.Layout(**layout_kwargs)
        )
        self.ZC2Y = ipw.FloatText(
            description="2Y", value=3.02, layout=ipw.Layout(**layout_kwargs)
        )
        self.ZC6Y = ipw.FloatText(
            description="6Y", value=4, layout=ipw.Layout(**layout_kwargs)
        )
        self.ZC10Y = ipw.FloatText(
            description="10Y", value=4.7, layout=ipw.Layout(**layout_kwargs)
        )

        self.normalVol = ipw.FloatSlider(
            value=100, min=0, max=1000, description="Normal Vol (bps)", width="250px"
        )
        self.normalVol2 = ipw.FloatSlider(
            value=100, min=0, max=1000, description="Normal Vol 2 (bps)", width="250px"
        )
        self.meanRev = ipw.FloatSlider(
            value=1, min=0.0001, max=10, steps=0.01, description="Mean Reversion", width="250px"
        )
        self.meanRev2 = ipw.FloatSlider(
            value=3, min=0.0001, max=10, steps=0.01, description="Mean Reversion 2", width="250px"
        )
        self.numPaths = ipw.IntSlider(
            value=50, min=1, max=500, step=1, description="Paths ", width="250px"
        )
        self.showPaths = ipw.IntSlider(
            value=5, min=0, max=100, step=1, description="Display Paths ", width="250px"
        )
        self.correlation = ipw.FloatSlider(
            value=0.1, min=-1, max=1, steps=0.01, description="Correlation", width="250px"
        )
        ...
        self.children = [
            ...
            ipw.VBox(
                [
                    ipw.HBox(
                        [self.normalVol, self.normalVol2, self.meanRev, self.meanRev2]
                    ),
        ...
        self.form = ipw.VBox(
            [
                ipw.HBox(curve_elements, layout={"width": "80%"}),
                self.tab,
            ]
        )
    ...
    def curveInit(self, click=None):
        ...
        ZCs = [
            [1 / 52, self.ZC1W.value],
            [0.25, self.ZC3M.value],
            [0.5, self.ZC6M.value],
            [1, self.ZC1Y.value],
            [2, self.ZC2Y.value],
            [6, self.ZC6Y.value],
        ...
        num = np.exp(curveZC["ZC rates"] / 100 * curveZC["dt"])
        denom = np.exp(curveZC["ZC_s"] / 100 * curveZC["dt_s"])
        curveZC["FWD rates"] = ((num / denom) - 1) / self.steps * 100
    ...
    def HW1F(self, click=None):
        ...
        data["t"] = pd.DataFrame(
            np.arange(0, 10 + self.steps, self.steps)
        )  # t0,t1,t2,t3...
        data["dT"] = data["dt"] - data["t"]  # time interval between periods
        data["Ft_0_t"] = (data["FWD rates"].shift(1) - data["FWD rates"]) / (
            data["dt"].shift(1) - data["dt"]
        )
        data["alpha_t"] = (
            np.power(sigma, 2) / (2 * a) * (1 - np.exp(-2 * a * data["t"]))
        )
    ...
    def HW2F(self, click=None):
        ...
        data["alpha_t"] = (
            np.power(sigma1, 2) / (2 * a) * (1 - np.exp(-2 * a * data["t"]))
        )
